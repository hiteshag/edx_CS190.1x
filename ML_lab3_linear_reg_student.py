...
def parsePoint(line):
    ...
    """
    splitStr = line.split(',')
    return LabeledPoint(splitStr[0], splitStr[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    mse = (labelsAndPreds
           .map(lambda (label, pred): squaredError(label, pred))
           .mean())
    return (mse ** 0.5)
...
def twoWayInteractions(lp):
    ...
    """
    prod = itertools.product(lp.features, repeat=2)
    feat = []
    for elem in prod:
        feat.append(elem[0] * elem[1])
    return LabeledPoint(lp.label, np.hstack((lp.features, feat)))
