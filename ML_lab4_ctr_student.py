...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    newFeats = map(lambda x: OHEDict[x], rawFeats)
    return SparseVector(numOHEFeats, sorted(newFeats), np.ones(len(newFeats)))
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    distinctInput = inputData.flatMap(lambda x: x).distinct()
    return distinctInput.zipWithIndex().collectAsMap()
...
def parsePoint(point):
    ...
    """
    feat = point.split(',')[1:]
    result = [(featId, value) for featId, value in enumerate(feat)]
    return result
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    ohePoint = oneHotEncoding(parsedPoint, OHEDict, numOHEFeats)
    return LabeledPoint(label, ohePoint)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    newFeats = map(lambda x: OHEDict[x], filter(lambda x: x in OHEDict, rawFeats))
    return SparseVector(numOHEFeats, sorted(newFeats), np.ones(len(newFeats)))
...
def computeLogLoss(p, y):
    ...
    epsilon = 10e-12
    if p == 0: p = epsilon
    elif p == 1: p = 1 - epsilon
    ll = -log(p) if y else -log(1-p)
    return ll
...
def evaluateResults(model, data):
    ...
    """
    dataPred = data.map(lambda lp: (getP(lp.features, model.weights, model.intercept), lp.label))
    logLoss = dataPred.map(lambda (prob, lab): computeLogLoss(prob, lab)).mean()
    return logLoss
...
def parseHashPoint(point, numBuckets):
    ...
    label = point.split(',')[0]
    parsedPoint = parsePoint(point)
    hashedPoint = hashFunction(numBuckets, parsedPoint, False)
    newFeats = hashedPoint.keys()
    hashedVec = SparseVector(numBuckets, sorted(newFeats), np.ones(len(newFeats)))
    return LabeledPoint(label, hashedVec)
